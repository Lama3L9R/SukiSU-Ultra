...
NativeBridge(getAppProfile, jobject, jstring pkg, jint uid) {
    ...
    jclass cls = GetEnvironment()->FindClass(env, "zako/zako/zako/Natives$Profile");
...
NativeBridge(setAppProfile, jboolean, jobject profile) {
    ...
    if (allowSu) {
        ...
        if (templateName) {
            const char* ctemplateName = GetEnvironment()->GetStringUTFChars(env, (jstring) templateName, NULL);
        ...
        const char* cdomain = GetEnvironment()->GetStringUTFChars(env, (jstring) domain, NULL);
